...
Tracker::Tracker() {
  ...
  // Agentpub = nh_.advertise<std_msgs::Float32MultiArray>
  // ("/tracked_agents", 400);
...
void Tracker::receiveTrackerData(const PTrackingBridge::
...
std::pair<float, Vector2> Tracker::calculateAvgMaxSpeeds(int AgentID,
    Vector2 AgentVel) {
  // TODO(Alex): Implement discounted sum over past Sum:(1 - disc)^t-1 * Value
  //   where disc(0 < disc =< 1)
  ...
  float avgVelx = 0.0f;
  float avgVely = 0.0f;
  for (std::vector<Vector2>::iterator vel = agentVelHistory_[AgentID].begin();
       vel != agentVelHistory_[AgentID].end(); ++vel) {
    avgSpeed += abs(*vel);
    avgVelx += (*vel).getX();
    avgVely += (*vel).getY();
  }
...
void Tracker::checkExistingTrackers(std::map<int, std::size_t> ids) {
  ...
  for (std::map<int, std::size_t>::iterator iter = trackedAgents_.begin();
    ...
    if (TrackID != robotTrackerID_ && AgentID == THIS_ROBOT && TRACK_ROBOTS) {
      ERR(environment_->getStringActorID() << " already has tracker " <<
          robotTrackerID_ << " instead of " << TrackID << std::endl);
...
void Tracker::updateActiveAgents(std::size_t numAgents) {
  // Cycle through trackers: Update existing agents
  // Create agents for new trackers
  // Save robot-odom comparison for each tracker
  for (std::size_t i = 0; i < numAgents; ++i) {
    ...
    if (INVERT_X) {
      ...
      agentVel = Vector2(-1 * msg_.averagedVelocities[i].x,
                         msg_.averagedVelocities[i].y);
    } else {
      agentPos = Vector2(msg_.positions[i].x, msg_.positions[i].y);
      agentVel = Vector2(msg_.averagedVelocities[i].x,
                         msg_.averagedVelocities[i].y);
    }

    // If only one robot is being used,
    //  assign it the tracker if it's the only one left
    if (ASSIGN_TRACKER_WHEN_ALONE && (trackedAgents_.empty() || numAgents == 1)
      ...
      DEBUG("Assigned lone tracker" << TrackerID << "to " <<
            environment_->getStringActorID() << std::endl);
    }

    // If new Tracker has not been assigned yet,
    // reassign to robot or create new agent
    if (trackOtherAgents_ &&
        trackedAgents_.find(TrackerID) == trackedAgents_.end() &&
        trackedAgents_.size() < MAX_NO_TRACKED_AGENTS ) {
      // TODO(Alex): Implement working limit for number of created agents
      if (TrackerID == robotTrackerID_ && ENABLE_PLANNER && TRACK_ROBOTS) {
        this->setAgentTracker(TrackerID, THIS_ROBOT);
        planner_->resetOdomPosition();
        DEBUG("Re-assigned robot tracker" << TrackerID << " to " <<
              environment_->getStringActorID() << std::endl);
      } else {
        trackedAgents_[TrackerID] = environment_-> addPedestrianAgent(
                                      "TrackedPerson" + sid, agentPos,
                                      environment_->addPlannerGoal(agentPos));
        DEBUG("New agent" << trackedAgents_[TrackerID] <<
              " with tracker" << sid << std::endl);
      }
    }
    ...
    if ((trackedAgents_[TrackerID] == THIS_ROBOT) && !ONLY_ODOMETRY &&
      ...
      std::pair<float, Vector2> avgSpeedVel = this->calculateAvgMaxSpeeds(
          trackedAgents_[TrackerID], agentVel);
      float avgSpeed = avgSpeedVel.first;
      Vector2 avgVel = avgSpeedVel.second;

      planner_->setAgentPosition(trackedAgents_[TrackerID],
                                 agentPos + trackerOffset);
      ...
      planner_->setAgentMaxSpeed(trackedAgents_[TrackerID],
                                 maxSpeed_[trackedAgents_[TrackerID]]);
    ...
    if (ENABLE_PLANNER && TRACK_ROBOTS) {
      ...
      // {
      trackerCompOdom_[TrackerID].insert(trackerCompOdom_[TrackerID].begin(),
                                         odomdiff);
      trackerCompOdom_[TrackerID].resize(TRACKER_ODOM_COMPARISONS);
      DEBUG("Tracker" << TrackerID << " Pos " << agentPos << std::endl);
      DEBUG("OdomPos:" << planner_->getOdomPosition() <<
            " CompOdom " << odomdiff << std::endl);
      // }
    }
  }
}
...
void Tracker::publishAgentLayer() {
  ...
  for (AgentPos = AgentPositions.begin(); AgentPos != AgentPositions.end();
       AgentPos++) {
...
void Tracker::odometryComparison() {
  ...
  if (ENABLE_PLANNER) {
    int TargetTrackerID =
      -1;  // -1 = No tracker is close enough to the robot odometry
    float minComp = 10.0f;
    ...
    for (std::map<int, std::vector<float> >::iterator iter =
           trackerCompOdom_.begin(); iter != trackerCompOdom_.end(); ++iter) {
      int TrackerID = iter->first;
      std::vector<float> OdomComparison = iter->second;
      if (planner_->getAgentType(trackedAgents_[TrackerID]) != INACTIVE) {
        odomSums[TrackerID] = 0.0f;
        for (std::vector<float>::iterator iter = OdomComparison.begin();
             iter != OdomComparison.end(); ++iter) {
          odomSums[TrackerID] += (*iter);
        }
        odomSums[TrackerID] = odomSums[TrackerID] / OdomComparison.size();
      }
    }
    for (std::map<int, float>::iterator iter = odomSums.begin();
         iter != odomSums.end(); ++iter) {
      int TrackerID = iter->first;
      float odomSum = iter->second;
      DEBUG("minComp " << minComp << " odomSum " << odomSum << " TrackerID " <<
            TrackerID << std::endl);
      if (odomSum < minComp) {
    ...
    if (TargetTrackerID == -1) {
      ERR("ERROR: NO TRACKER CAN BE ASSIGNED TO " <<
          environment_->getStringActorID() << std::endl);
    } else if (trackedAgents_[TargetTrackerID] != THIS_ROBOT &&
               trackerCompOdom_[TargetTrackerID].size()
               >= TRACKER_ODOM_COMPARISONS) {
      ERR("Tracker " << TargetTrackerID << "belongs to " <<
          environment_->getStringActorID() << " instead of " <<
          trackedAgents_[TargetTrackerID] << std::endl);
      // If smallest is not the one assigned to robot,
      // then substitute and delete extra tracked agent
